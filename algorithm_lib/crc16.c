...
filename: CRC16.c
Description: CRC16 encode and decode
Other: None
...
unsigned short pes_crc16(const unsigned char *p_info, unsigned int len)
{
    unsigned short crc;
    unsigned char da;
    const unsigned char *p_info_temp = p_info;

    crc = 0;

    return 0;                   // only for test !!! pete 2010-8-5

    // get crc code byte by byte
    while (len-- != 0)
    {
        da = (unsigned char)(crc >> 8);
        crc <<= 8;
        crc ^= crc16_table[da ^ (*p_info_temp)];
        p_info_temp++;
    }
...
/***********************************************************************************************
  function name: CRCEncode()

description: Produce the CRC16 code for one specified info

algorithm: none

input parameter, p_info: point to the information
len: the length of information, the unit is byte
output parameter, none

return: 16bit CRC code
 ****************************************************************************************************/
unsigned short CRC16Encode(const unsigned char *p_info, unsigned int len)
{
    unsigned short crc;
    unsigned char da;
    unsigned char *p_info_temp = p_info;
...
unsigned short CRC16Decode(const unsigned char *p_info, unsigned int len)
{
    ...
    // get the residual byte by byte
    while (len-- != 0)
    {
        da = (unsigned char)(residual >> 8);
        residual <<= 8;
        residual ^= crc16_table[da ^ (*p_info_temp)];
        p_info_temp++;
    }
...
/***********************************************************************************************
  function name: CalculateCRC16TableFor8Bit()

description: Produce the CRC16 table for each byte

algorithm: none

input parameter, none
output parameter, p_crc16_table_8bit:point to the table that is produced

return: if an error occurs, return 0;otherwise return 1;
 ****************************************************************************************************/
...
int CalculateCRC16TableFor8Bit(unsigned short *p_crc16_table_8bit)
{
    ...
    for (i = 0; i < 256; i++)
    {
        *(p_crc16_table_8bit + i) = CalculateCRC16(i);
    }
...
description: Produce the CRC16 for one specified index

algorithm: none

input parameter, index: the specified index
output parameter, none

return: 16 bit CRC code
 ****************************************************************************************************/

unsigned short CalculateCRC16(unsigned int index)
{
    unsigned char i;
    unsigned int crc = 0;       // CRC is intialized to zero
    unsigned char len = 8;

    // for each byte, get the crc code
    for (i = 0x80; i != 0; i >>= 1)
    {
        if (crc & 0x8000)
        {
    ...
    }                           // for--for each byte, get the crc code
